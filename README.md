...
**Shareable, restorable app state via the URL.**
Selecting a listing writes `?id=<propertyId>` into the URL (`history.replaceState`, not `pushState`, so browsing around doesn't spam the back button); opening that link recenters the map and selects it before the first render. Every filter (search, district, price range, sort, which deal-rating colors are shown) is mirrored the same way (`q`, `district`, `price`, `sort`, `deals`), and restored from the URL on load — so a *filtered* view, not just a single listing, is a shareable link. A parameter is only written when it differs from the default, so the common case (no filters applied) stays a clean URL.

**One shared mortgage calculator instead of one per card.**
Down payment / interest rate / loan term live once, in the settings panel, and apply to every currently-rendered card — adjusting a value does a targeted DOM update (`updateCalcLines()`) rather than a full re-render, so it doesn't replay card animations or lose scroll position. The same inputs feed the comparison table's "estimated payment" row, so the two features share one source of truth instead of computing it twice.

**Listing comparison without a backend.**
Selecting up to 3 listings (⚖️ button per card) is pure client-side `Set` state — no table, no round trip. A floating bar tracks the selection and opens a modal with a side-by-side table (district, price, area, Kč/m², AI rating, estimated payment).
